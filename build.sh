...
echo -e "ARCHIVE PATH: ${archive_path}"
...
gcc_archive_path=${archive_path}/gcc/gcc-${gcc_version}.tar.bz2
gmp_archive_path=${archive_path}/gmp/gmp-${gmp_version}.tar.bz2
mpfr_archive_path=${archive_path}/mpfr/mpfr-${mpfr_version}.tar.bz2
mpc_archive_path=${archive_path}/mpc/mpc-${mpc_version}.tar.gz
isl_archive_path=${archive_path}/isl/isl-${isl_version}.tar.bz2
cloog_archive_path=${archive_path}/cloog/cloog-${cloog_version}.tar.gz
...
if [ ! -d gcc-${gcc_version} ]; then
    echo -e "Extracting GCC-${gcc_version} from ${gcc_archive_path}..."
...
if [ -d objdir ]; then
    ...
    mkdir objdir
    cd objdir
    ../gcc-${gcc_version}/configure --prefix=${install_path} --enable-languages=c,c++ --with-pic --disable-shared --enable-static --enable-threads=posix --enable-__cxa_atexit --enable-clocale=gnu --with-libelf=/usr/include --disable-multilib --disable-bootstrap --disable-install-libiberty --with-system-zlib
fi
